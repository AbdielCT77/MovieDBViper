...
enum BaseError: Error {
    ...
    public var getError: (ErrorProtocol) {
        switch self {
        case .networkError:
            return GenericAlert(errCode: "1000",
                                         title: "Failed",
                                         desc: "Network Error")
        ...
        case .apiFailure(let json):
            ...
            if msg.contains("|"){
                ...
                return GenericAlert(errCode: code,
        ...
        case .custom(let code, let title, let desc):
            return GenericAlert(errCode: code, title: title, desc: desc)
        default:
            return GenericAlert(errCode: "0",
                                title: "Failed",
    ...
    private func getHttpErrorMessage(httpCode: Int) -> (ErrorProtocol)
    {
        ...
        if (httpCode >= 300 && httpCode <= 309) {
            // Redirection
            return GenericAlert(errCode: String(httpCode),
                                title: "Failed",
...
struct GenericAlert : ErrorProtocol, Error {
    var errCode = ""
    var title = ""
    var desc = ""
